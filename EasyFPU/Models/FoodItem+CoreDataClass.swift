...
public class FoodItem: NSManagedObject {
    ...
    static func getCalories(foodItem: FoodItem, amount: Double) -> Double {
        amount / 100 * foodItem.caloriesPer100g
    }
    
    static func getCarbsInclSugars(foodItem: FoodItem, amount: Double) -> Double {
        amount / 100 * foodItem.carbsPer100g
    }
    
    static func getSugarsOnly(foodItem: FoodItem, amount: Double) -> Double {
        amount / 100 * foodItem.sugarsPer100g
    }
    
    static func getRegularCarbs(foodItem: FoodItem, amount: Double, treatSugarsSeparately: Bool) -> Double {
        amount / 100 * (treatSugarsSeparately ? (foodItem.carbsPer100g - foodItem.sugarsPer100g) : foodItem.carbsPer100g)
    }
    
    static func getSugars(foodItem: FoodItem, amount: Double, treatSugarsSeparately: Bool) -> Double {
        amount / 100 * (treatSugarsSeparately ? foodItem.sugarsPer100g : 0)
    }
    
    static func getFPU(foodItem: FoodItem, amount: Double) -> FPU {
        // 1g carbs has ~4 kcal, so calculate carb portion of calories
        let carbsCal = amount / 100 * foodItem.carbsPer100g * 4;

        // The carbs from fat and protein is the remainder
        let calFromFP = getCalories(foodItem: foodItem, amount: amount) - carbsCal;
